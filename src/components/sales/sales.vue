  ...
  <div>
    <el-form ref="form" :rules="rule" :model="form" label-width="80px">
      ...
      <el-form-item label="产品种类">
        ...
        <el-select @change="selGoods" v-model="form.goods_id" name="goods_id" placeholder="请选择种类">
          <el-option
      ...
      <el-form-item prop="purch_number" label="销售数量(件)">
  ...
  data() {
    var validate = (rule, value, callback) => {
      if (value > this.max_num) {
        callback(new Error(`库存只有${this.max_num}`));
      }
        callback();
    };
    return {
  ...
  created() {
    this.axios('/goods/selAll').then( (res) => {
      ...
      if(res.data.status == 200){
        ...
        this.selGoods(this.options[0].id)
      }
    })
  },
  methods: {
    selGoods(val){
      this.max_num = this.map.get(val).goodsNumber
      this.form.purch_number = 1
    },
    onSubmit() {
